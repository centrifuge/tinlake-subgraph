...
import { NftFeed } from "../../generated/NftFeed/NftFeed"
...
export function handleShelfIssue(call: IssueCall): void {
  ...
  log.debug(
    "handleShelfIssue, shelf: {}, loanOwner: {}, loanIndex: {},  nftId: {}, nftRegistry: {}",
    [
      shelf.toHex(),
      loanOwner.toHex(),
      loanIndex.toString(),
  ...
  if (nftHash.reverted) {
    if (poolMeta.id == "0x382460db48ee1b84b23d7286cfd7d027c27bb885") {
      log.error("failed to find nft hash for loan idx {}", [
        loanIndex.toString(),
      ]);
    } else {
      log.critical("failed to find nft hash for loan idx {}", [
        loanIndex.toString(),
      ]);
    }
    return;
  ...
  if (riskGroup.reverted) {
    if (poolMeta.id == "0x382460db48ee1b84b23d7286cfd7d027c27bb885") {
      log.error("failed to find risk group for nft hash {}", [
        nftHash.value.toString(),
      ]);
    } else {
      log.critical("failed to find risk group for nft hash {}", [
        nftHash.value.toString(),
      ]);
    }
    return;
...
export function handleShelfBorrow(call: BorrowCall): void {
  ...
  log.debug(
    "handleShelfBorrow, shelf: {}, loanOwner: {}, loanIndex: {}, amount: {}",
    [shelf.toHex(), loanOwner.toHex(), loanIndex.toString(), amount.toString()]
  );
  ...
  pool.totalBorrowsAggregatedAmount = pool.totalBorrowsAggregatedAmount.plus(
    amount
  );
...
export function handleShelfRepay(call: BorrowCall): void {
  ...
  log.debug(
    "handleShelfRepay, shelf: {}, loanOwner: {}, loanIndex: {}, amount: {}",
    [shelf.toHex(), loanOwner.toHex(), loanIndex.toString(), amount.toString()]
  );
  ...
  pool.totalRepaysAggregatedAmount = pool.totalRepaysAggregatedAmount.plus(
    amount
  );
